...
export function ErrorState({ error }: { error: unknown }) {
  if (error instanceof BackendDownError) {
    return (
      ...
      >
        <span className="grid size-9 shrink-0 place-items-center rounded-full border border-pain-muted bg-canvas text-pain">
  ...
  return (
    <div
      role="alert"
      className="flex items-start gap-3 rounded-ui border border-pain-muted bg-pain-soft px-4 py-3 text-sm text-pain"
    >
