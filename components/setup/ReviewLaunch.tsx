...
export function ReviewLaunch({
  ...
  async function launch() {
    ...
    try {
      ...
      // Save campaign name if provided
          ...
          <motion.aside {...reveal(0.14)}>
            <Card>
              ...
              <CardContent className="flex flex-col gap-5">
                {/* Campaign name — optional, falls back to "Kampagne N" on overview */}
                <div className="flex flex-col gap-1.5">
                  <label
                    htmlFor="campaign-name"
                    className="text-[length:var(--text-caption)] font-semibold tracking-[0.04em] text-fg-subtle"
                  >
                    Kampagnenname <span className="font-normal text-fg-faint">(optional)</span>
                  </label>
                  <input
                    id="campaign-name"
                    type="text"
