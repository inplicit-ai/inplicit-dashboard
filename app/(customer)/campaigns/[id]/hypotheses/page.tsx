...
const STATE_TINT: Record<string, string> = {
  VERIFIED: "border-success/30 bg-success-soft/40",
  REJECTED: "border-pain/30 bg-pain-soft/40",
  EVOLVING: "border-warning/30 bg-warning/5",
};
...
function HypothesisCard({ h }: { h: Hypothesis }) {
  ...
  const tint = STATE_TINT[h.validation_state] ?? "border-line bg-surface";

  return (
    <Card
      className={cn(
        "rounded-card p-6 transition-colors hover:border-line-strong",
        tint,
      )}
    >
      ...
      {total > 0 && (
        <>
          <div className="mt-4 h-1 w-full overflow-hidden rounded-full bg-pain-soft">
            <div
      ...
      {h.dept_coverage.length > 0 && (
        <div className="mt-4 flex flex-wrap gap-2">
          {h.dept_coverage.map((d) => (
            <Badge key={d} variant="secondary" className="font-medium">
              {d}
            </Badge>
          ))}
        </div>
